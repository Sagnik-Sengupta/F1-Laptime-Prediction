...
spark = SparkSession.builder.appName("F1_Q3_Prediction").getOrCreate()

seasons = range(2018, 2025)

for year in seasons:
    data_path = f"f1_data_{year}.csv"
    ...
    predictions = predictions.orderBy("Predicted_Q3")
    output_path = f"predicted_q3_{year}_last_race.csv"
    predictions.coalesce(1).write.csv(output_path, header=True, mode="overwrite")
    print(f"Season {year}: Predictions saved to directory '{output_path}'.")
